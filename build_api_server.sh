...
docker build -t balopat/planqtn_api:${TAG} --file=Dockerfile.api .

echo "Image built successfully"

if [ "$PUSH" = true ]; then
    echo "Pushing image to registry..."
    docker push balopat/planqtn_api:${TAG}
    echo "Image pushed successfully"
fi

if [ "$RUN" = true ]; then
    echo "Running container with attached TTY..."
    docker run -t --rm -p 5173:5173 balopat/planqtn_api:${TAG}
elif [ "$RUND" = true ]; then
    echo "Running container in detached mode..."
    CONTAINER_ID=$(docker run -d --rm -p 5173:5173 balopat/planqtn_api:${TAG})
    echo "Container started with ID: ${CONTAINER_ID}"
    echo "Tailing logs (Ctrl+C to stop)..."
    docker logs -f ${CONTAINER_ID}
elif [ "$PUSH" != true ]; then
    echo "To push the image to the registry, run:"
    echo "    docker push balopat/planqtn_api:${TAG}"    
