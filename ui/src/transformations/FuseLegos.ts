...
export class FuseLegos {
    ...
    public async apply(legosToFuse: DroppedLego[]): Promise<{ connections: Connection[], droppedLegos: DroppedLego[], operation: Operation }> {
        try {
            ...
            const newLego: DroppedLego = {
                ...
                instanceId: (Math.max(...this.droppedLegos.map(l => parseInt(l.instanceId))) + 1).toString(),
                shortName: "Fused",
                name: "Fused Lego",
            ...
            const resultingDroppedLegos = [
                ...this.droppedLegos.filter(l => !legosToFuse.some(fl => fl.instanceId === l.instanceId)),
                newLego
            ];
