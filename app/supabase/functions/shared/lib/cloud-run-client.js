...
exports.CloudRunClient = exports.cloudRunHeaders = void 0;
exports.getIdTokenClientForCloudRun = getIdTokenClientForCloudRun;
const npm_google_auth_library_9_15_1_1 = require("npm:google-auth-library@9.15.1");
const base64_1 = require("jsr:@std/encoding/base64");
const cloudRunHeaders = async (backendUrl) => {
    return await getIdTokenClientForCloudRun(backendUrl).then((idTokenClient) => idTokenClient.getRequestHeaders(backendUrl));
};
exports.cloudRunHeaders = cloudRunHeaders;
const googleCredentials = () => {
    const serviceAccountBase64 = Deno.env.get("SVC_ACCOUNT");
    if (!serviceAccountBase64) {
        throw new Error("SVC_ACCOUNT environment variable not set");
    }
    const decodedString = new TextDecoder().decode((0, base64_1.decodeBase64)(serviceAccountBase64));
    return JSON.parse(decodedString);
};
async function getIdTokenClientForCloudRun(audience) {
    try {
        const auth = new npm_google_auth_library_9_15_1_1.GoogleAuth({
            credentials: googleCredentials(), // Cast to any to bypass strict library typing for now
        });
...
class CloudRunClient {
    constructor(projectId, location) {
        console.log(`CloudRunClient constructor called with projectId: '${projectId}', location: '${location}'`);
        console.log("Deno env vars", Deno.env.toObject());
        this.projectId = projectId;
        this.location = location;
        const credentials = googleCredentials();
        console.log("Credentials", credentials);
        this.googleAuth = new npm_google_auth_library_9_15_1_1.GoogleAuth({
            credentials: credentials, // Cast to any to bypass strict library typing for now
        ...
        console.log("Initializing CloudRunClient (REST API version) with projectId:", projectId, "location:", location);
    }
    async createJob(jobType, command, args, config, serviceAccountName, postfix, env) {
        ...
        try {
            ...
            if (!response.ok) {
                console.error("Error response from REST API creating job:", responseBody);
                const errorMessage = responseBody.error?.message ||
                    JSON.stringify(responseBody);
                throw new Error(`Failed to run job via REST API. Status: ${response.status} ${response.statusText}. ` +
    ...
    async pollLRO(lroName, maxAttempts = 10, delayMs = 5000) {
        console.log(`Polling LRO: ${lroName}`);
        const lroApiUrl = `https://run.googleapis.com/v2/${lroName}`;
        let attempts = 0;
        while (attempts < maxAttempts) {
            attempts++;
            console.log(`LRO Poll attempt ${attempts}/${maxAttempts} for ${lroName}`);
            const token = await this.googleAuth.getAccessToken();
            if (!token) {
                throw new Error("Failed to retrieve access token for LRO polling.");
            }
            ...
            const lroStatus = await response.json();
            if (!response.ok) {
                console.error("Error polling LRO:", lroStatus);
                const errorMessage = lroStatus.error?.message ||
                    JSON.stringify(lroStatus);
                throw new Error(`Failed to poll LRO. Status: ${response.status} ${response.statusText}. ` +
                    `Response: ${errorMessage}`);
            }
            if (lroStatus.done) {
        ...
        throw new Error(`LRO ${lroName} did not complete after ${maxAttempts} attempts.`);
    }
    // Helper to extract the short execution ID from the full execution resource name
    getShortExecutionId(fullExecutionResourceName) {
        if (!fullExecutionResourceName ||
            !fullExecutionResourceName.includes("/")) {
            console.warn(`Invalid fullExecutionResourceName for getShortExecutionId: ${fullExecutionResourceName}`);
            return fullExecutionResourceName; // Return as is if not a valid path
        }
        return fullExecutionResourceName.substring(fullExecutionResourceName.lastIndexOf("/") + 1);
    }
    async getJobLogs(lroName, defaultJobNameForFilter = "planqtn-jobs") {
        ...
        if (completedLro.error) {
            console.warn(`Warning: The job associated with LRO ${lroName} reported an error: ${completedLro.error.message}. Attempting to fetch logs anyway. Details:`, JSON.stringify(completedLro.error.details || {}, null, 2));
        ...
        if (!completedLro.metadata) {
            throw new Error(`LRO ${lroName} did not yield metadata containing execution details. Cannot determine execution to fetch logs for.`);
        }
        const executionDetails = completedLro.metadata;
        const executionResourceName = executionDetails.name;
        if (!executionResourceName) {
            throw new Error(`Could not determine execution resource name from LRO metadata for ${lroName}.`);
        }
        ...
        const jobNameFromLabel = executionDetails.labels?.["run.googleapis.com/job_name"];
        ...
        console.log(`Using jobName: '${jobName}' for log filter (derived from metadata: ${!!jobNameFromMetadata}, label: ${!!jobNameFromLabel}).`);
        if (executionDetails.logUri) {
            console.log(`Console Log URI for this execution: ${executionDetails.logUri}`);
        }
        // 3. Proceed with fetching logs using the executionResourceName
        const shortExecutionId = this.getShortExecutionId(executionResourceName);
        console.log(`Fetching logs for execution ID: ${shortExecutionId} (from resource ${executionResourceName}) of job: ${jobName}`);
        ...
        const filter = `labels."run.googleapis.com/execution_name"="${shortExecutionId}"\n` +
        ...
        try {
            ...
            do {
                ...
                if (!response.ok) {
                    ...
                    const errorMessage = responseBody.error?.message ||
                        JSON.stringify(responseBody);
                    throw new Error(`Failed to get logs from Logging API. Status: ${response.status} ${response.statusText}. ` +
                        `Response: ${errorMessage}`);
                }
                ...
                console.log(`Retrieved ${responseBody.entries?.length || 0} log entries. Total so far: ${allEntries.length}. Has next page: ${!!nextPageToken}`);
            } while (nextPageToken);
            ...
            if (error instanceof Error) {
                ...
                console.error("Caught an unknown error type during log fetching:", error);
            }
            throw error;
        }
    }
    async cancelJob(lroName) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const errorMessage = responseBody.error?.message ||
                    JSON.stringify(responseBody);
                throw new Error(`Failed to get LRO details. Status: ${response.status} ${response.statusText}. ` +
                    `Response: ${errorMessage}`);
            }
            const lroStatus = await response.json();
            if (!lroStatus.metadata) {
                throw new Error(`LRO ${lroName} did not yield metadata containing execution details. Cannot determine execution to cancel.`);
            }
            const executionDetails = lroStatus.metadata;
            const executionResourceName = executionDetails.name;
            if (!executionResourceName) {
                throw new Error(`Could not determine execution resource name from LRO metadata for ${lroName}.`);
            }
            // Construct the cancel API URL for the execution
            const cancelApiUrl = `https://run.googleapis.com/v2/${executionResourceName}:cancel`;
            ...
            if (!cancelResponse.ok) {
                ...
                const errorMessage = responseBody.error?.message ||
                    JSON.stringify(responseBody);
                throw new Error(`Failed to cancel execution. Status: ${cancelResponse.status} ${cancelResponse.statusText}. ` +
                    `Response: ${errorMessage}`);
            }
            console.log(`Successfully initiated cancellation for execution: ${executionResourceName}`);
            ...
            while (attempts < maxAttempts) {
                attempts++;
                console.log(`Checking cancellation status (attempt ${attempts}/${maxAttempts})...`);
                const statusResponse = await fetch(`https://run.googleapis.com/v2/${executionResourceName}`, {
                    method: "GET",
                    headers: {
                        "Authorization": `Bearer ${token}`,
                        "Content-Type": "application/json",
                ...
                const isCancelling = executionStatus.conditions?.some((condition) => condition.type === "Completed" &&
            ...
            console.warn(`Cancellation initiated but could not confirm status after ${maxAttempts} attempts.`);
        }
        catch (error) {
